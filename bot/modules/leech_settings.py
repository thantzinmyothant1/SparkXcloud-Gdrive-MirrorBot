...
def getleechinfo(from_user):
    ...
    text = f"𝐋𝐞𝐞𝐜𝐡 𝐒𝐞𝐭𝐭𝐢𝐧𝐠𝐬 𝐟𝐨𝐫 <u><a href='tg://user?id={user_id}'>{name}</a></u>\n"\
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        msg = f"𝐂𝐮𝐬𝐭𝐨𝐦 𝐭𝐡𝐮𝐦𝐛𝐧𝐚𝐢𝐥 𝐬𝐚𝐯𝐞𝐝 𝐟𝐨𝐫 <a href='tg://user?id={user_id}'>{update.message.from_user.full_name}</a>."
